...
if ( !class_exists( 'pre_spam_checker' ) ) {
	class pre_spam_checker {
		...
		protected static function endpoint_request( $_update = true ) {
			...
			$url .= urlencode( self::get_protocol() . $_SERVER[ 'HTTP_HOST' ] . $_SERVER[ 'REQUEST_URI' ] ) . '&agent=' . urlencode( self::get_user_agent() ) . '&v=1&p=5&ip=' . urlencode( $_SERVER[ 'REMOTE_ADDR' ] ) . '&siteurl=' . urlencode( get_site_url() );
