...
//print 'max_form_num:'.self::$global_options['max_form_num']."<br>\n";
	...
	static function convert_form_options( $old_options, $max_fields ) {
		...
		// This assumes that the standard fields in the form defaults are in the same order as the names in the above array
		foreach ( $std_fields as $key => $val ) {
			...
			if ( $new_options['fields'][$key]['slug'] == $test ) {
				...
				// This could happen if the standard fields in the default form are in a different order than in $std_fields
			}
